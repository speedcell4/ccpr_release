...
class TranslationTemplates:
    ...
    def fewshot_plain(src_lang, tgt_lang, src_text, fewshot_examples):
        fewshot_template = "{}:\n{}\n{}:\n{}"
        fewshot_examples = [fewshot_template.format(src_lang, ex_src, tgt_lang, ex_tgt) for ex_src, ex_tgt in
                            fewshot_examples]
        input_template = "{}:\n{}\n\n{}:"
        input_text = input_template.format(src_lang, src_text, tgt_lang)
    ...
    def phrase(src_lang, tgt_lang, src_text, phrase_examples):
        ...
        phrase_examples = [fewshot_template.format(src_lang, src_phrase, "\t".join(tgt_phrases)) for
                           src_phrase, tgt_phrases in phrase_examples]
        input_template = "{}:\n{}\n\n{}:"
        input_text = input_template.format(src_lang, src_text, tgt_lang)
    ...
    def phrase_with_context(src_lang, tgt_lang, src_text, phrase_examples):
        fewshot_template = "{} Phrase: {}\nPotential Translation: {}\nContext: {}"
        phrase_examples = [fewshot_template.format(src_lang, src_phrase, tgt_phrase, phrase_ctx) for
                           src_phrase, tgt_phrase, phrase_ctx in phrase_examples]
        input_template = "{}:\n{}\n\n{}:"
        input_text = input_template.format(src_lang, src_text, tgt_lang)
    ...
    def chunk(src_lang, tgt_lang, src_text, chunk_examples):
        ...
        chunk_examples = [fewshot_template.format(src_lang, src_phrase, tgt_lang, "\n".join(tgt_chunks)) for
                          src_phrase, tgt_chunks in chunk_examples]
        input_template = "{}:\n{}\n\n{}:"
        input_text = input_template.format(src_lang, src_text, tgt_lang)
...
if __name__ == "__main__":
    ...
    chunks = [("Wahlsieg im Mai", ("from 1 votes July from the", "before the handover in 1997, when")),
              ("Auslandsbesuch", ("a rare overseas trip, are", "the last foreign trips for the"))]
    phrase_with_context = [("Premierminister", "Prime Ministers",
